...
_ROWS: list[tuple[str, str, str, str, str | None, bool]] = [
    ...
    ("core", "metabolic.fasting_plasma_glucose", "Fasting plasma glucose", "laboratory", "mmol/L", True),
    ...
    ("extended", "metabolic.body_fat_percent", "Body fat percentage", "body_composition", "%", False),
    ("extended", "metabolic.c_peptide", "C-peptide", "laboratory", "nmol/L", False),
    ("extended", "metabolic.cgm_glucose_cv", "CGM glucose coefficient of variation", "derived", "%", False),
    ...
    ("extended", "metabolic.ogtt_2h_glucose", "OGTT 2-hour plasma glucose", "laboratory", "mmol/L", False),
    ...
    ("extended", "immune.eosinophils_abs", "Absolute eosinophil count", "laboratory", "10*9/L", False),
    ...
    ("research", "immune.inflammation_proteomics_panel", "Inflammation proteomics panel", "omics", None, False),
    ...
    ("extended", "cardio.cf_pwv", "Carotid-femoral pulse wave velocity", "functional_test", "m/s", False),
    ("extended", "cardio.cac_score", "Coronary artery calcium score", "imaging", "{Agatston}", False),
    ("extended", "cardio.mean_arterial_pressure", "Mean arterial pressure", "derived", "mm[Hg]", False),
    ("extended", "cardio.pulse_pressure", "Pulse pressure", "derived", "mm[Hg]", False),
    ("specialist", "cardio.central_systolic_bp", "Central systolic blood pressure", "vital_sign", "mm[Hg]", False),
    ("research", "cardio.coronary_plaque_volume", "Coronary plaque volume", "imaging", "mm3", False),
    # --- Neurocognitive (21) ---
    ("core", "neuro.cognitive_composite_score", "Cognitive battery composite score", "functional_test", "{score}", True),
    ("core", "neuro.episodic_memory_score", "Episodic memory score", "functional_test", "{score}", True),
    ("core", "neuro.executive_function_score", "Executive function score", "functional_test", "{score}", True),
    ("core", "neuro.processing_speed_score", "Processing speed score", "functional_test", "{score}", True),
    ...
    ("extended", "neuro.hearing_pure_tone_average", "Pure-tone hearing average", "functional_test", "dB[HL]", False),
    ...
    ("extended", "neuro.visual_acuity_logmar", "Visual acuity", "functional_test", "{logMAR}", False),
    ("extended", "neuro.vitamin_b12", "Vitamin B12", "laboratory", "pmol/L", False),
    ("extended", "neuro.working_memory_score", "Working memory score", "functional_test", "{score}", False),
    ("specialist", "neuro.abeta42_40_ratio", "Plasma amyloid beta 42/40 ratio", "laboratory", "1", False),
    ...
    ("core", "regen.egfr_creatinine", "eGFR, creatinine-based", "derived", "mL/min/{1.73_m2}", True),
    ...
    ("core", "regen.uacr", "Urine albumin-to-creatinine ratio", "laboratory", "mg/mmol{creat}", True),
    ("core", "regen.vo2peak_direct", "VO2peak, directly measured", "functional_test", "mL/(kg.min)", True),
    ("extended", "regen.reticulocytes_abs", "Absolute reticulocyte count", "laboratory", "10*9/L", False),
    ("extended", "regen.cystatin_c", "Cystatin C", "laboratory", "mg/L", False),
    ("extended", "regen.egfr_creatinine_cystatin", "eGFR, creatinine-cystatin C", "derived", "mL/min/{1.73_m2}", False),
    ("extended", "regen.heart_rate_recovery_1min", "Heart rate recovery at 1 minute", "derived", "/min", False),
    ("extended", "regen.hrv_sdnn", "Heart rate variability SDNN", "wearable", "ms", False),
    ("extended", "regen.nocturnal_spo2_mean", "Mean nocturnal oxygen saturation", "wearable", "%", False),
    ("extended", "regen.mvpa_minutes_per_day", "Moderate-to-vigorous activity", "wearable", "min/d", False),
    ("extended", "regen.nocturnal_respiratory_rate", "Nocturnal respiratory rate", "wearable", "/min", False),
    ...
    # --- Musculoskeletal (23) ---
    ("core", "msk.appendicular_lean_mass", "Appendicular lean mass", "body_composition", "kg", True),
    ...
    ("extended", "msk.femoral_neck_bmd", "Femoral neck bone mineral density", "imaging", "g/cm2", False),
    ("extended", "msk.femoral_neck_t_score", "Femoral neck T-score", "imaging", "{T-score}", False),
    ("extended", "msk.lumbar_spine_bmd", "Lumbar spine bone mineral density", "imaging", "g/cm2", False),
    ...
    ("extended", "msk.sppb_total", "Short Physical Performance Battery total score", "functional_test", "{score}", False),
    ("extended", "msk.single_leg_stance", "Single-leg stance time", "functional_test", "s", False),
    ("extended", "msk.six_min_walk_distance", "Six-minute walk distance", "functional_test", "m", False),
    ...
    ("specialist", "msk.ctx", "C-terminal telopeptide of type I collagen", "laboratory", "ng/L", False),
    ("specialist", "msk.p1np", "Procollagen type I N-terminal propeptide", "laboratory", "ug/L", False),
]
