...
1. **Clinic creates / invites a patient** — `patients` service writes a `tenant_id`-scoped, `is_synthetic`-flagged patient row. Patient creation is **gated on consent status**.
2. **Consent is recorded** — `consents` service writes a versioned, purpose-scoped `ConsentRecord` (text version, purposes, timestamp, channel). Consent gates patient creation, data-source connection, and patient release. Withdrawal is attributable and triggers defined downstream handling (pending: full retention/deletion automation).
